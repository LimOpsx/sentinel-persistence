...
public abstract class InMemoryRuleRepositoryAdapter<T extends RuleEntity> implements RuleRepository<T, Long> {
    ...
    public T save(T entity) {
        ...
        if (processedEntity != null) {
            ...
            String app = processedEntity.getApp();
            app = StringUtils.isBlank(app) ? "":app;
            machineRules.computeIfAbsent(MachineInfo.of(app, processedEntity.getIp(),
                    processedEntity.getPort()), e -> {
                return new ConcurrentHashMap<>(32);
            })
                    .put(processedEntity.getId(), processedEntity);
            appRules.computeIfAbsent(app, v -> {
                return new ConcurrentHashMap<>(32);
            })
                    .put(processedEntity.getId(), processedEntity);
        }
